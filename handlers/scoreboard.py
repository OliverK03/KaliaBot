...
from utils.storage import get_scoreboard


async def build_scoreboard_text(
    ...
    for index, (user_id, count) in enumerate(rows, start=1):
        ...
        lines.append(f"{prefix} {display_name} — {count}")
...
async def scoreboard_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rows = get_scoreboard(chat_id)
    text = await build_scoreboard_text(
        context,
        chat_id,
        rows,
        "Isoimmat kalia-nautiskelijat",
